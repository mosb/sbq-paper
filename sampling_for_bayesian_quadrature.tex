...
Problems of numerical integration occur in many different contexts, including the computation of model evidences or normalisation constants. Model-based integration, or Bayesian Quadrature, offers many advantages over standard Monte Carlo approaches. These include sample efficiency and an estimate of the uncertainty in the integral.  We extend existing Bayesian Quadrature models by introducing a positivity constraint for the integrand and by approximately marginalising over the model's hyperparameters in closed form. We also introduce an active learning scheme to select our function samples, rather than running a Markov chain. We demonstrate our method on a real model comparison problem from biological science.
\end{abstract} 
