...
%	\centering
	...
	\caption{This figure shows the posterior distribution over $Z$ for several methods as the number of samples increases.  Shaded regions denote $\pm2$ SD's from the mean.  The shaded regions for \acro{smc} and \acro{bmc} are off the vertical scale of this figure.}
\label{fig:se}
\end{figure}
...
\begin{figure}
	...
	\caption{The log density of the true evidence over a one-dimensional example for different methods (colours identical to those in Figure \ref{fig:se}), compared to the true $Z$ (in black).  The integrand is the same as that in Figure \ref{fig:eue}.}
\label{fig:nll}
\end{figure}
