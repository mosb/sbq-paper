...
Problems of numerical integration occur in many different contexts, including the computation of model evidences or normalisation constants. Model-based integration, or Bayesian Quadrature, offers many advantages over standard Monte Carlo approaches. These include sample efficiency and an estimate of the uncertainty in our integral.  We extend existing Bayesian Quadrature models by introducing a positivity constraint for our integrand and by approximately marginalising over our model's hyperparameters in closed form. We also introduce an active learning scheme to select our function samples, as opposed to running a Markov chain. We demonstrate our method on a real model comparison problem from biological science.
\end{abstract} 
...
\paragraph{Metrics:} We judged our methods according to three metrics, all averages over $N$ similar experiments indexed by $i$. Define $Z_i$ as the ground truth evidence for the $i$th experiment, $m(Z_i)$ as its estimated mean  and $V(Z_i)$  as its predicted variance. Firstly, we computed the normalised mean square error,
$
\acro{nmse} \deq 10 \log_{10} \nicefrac{1}{N} \sum_{i=1}^{N} \nicefrac{(m(Z_i) - Z_i)^2}{Z_i^2}\,,
$
providing the average percentage error, in decibels. Next we computed the log-density of the truth, assuming experiments are independent,
$
...
\section{Conclusions}
 ...
 In this paper, we have made several advances to the \acro{bq} method for evidence estimation, including a positivity constraint and approximate hyperparameter marginalisation.  We have also demonstrated the many advantages of model-based integration over standard \acro{mc}: a natural stopping criterion, an estimate of uncertainty in our integral, and the ability to use active learning to select our samples. 
...
While we do not expect model-based integration to be a better choice in every instance, we expect that this family of approaches will come to be seen as a standard alternative to \acro{mc}.
