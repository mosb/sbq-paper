...
One solution to this intractability is found in \emph{simple Monte Carlo} (\acro{smc}). \acro{smc} generates samples $x_1 \dots x_N$ from the prior distribution, and estimates $Z$ by $$\hat{Z} = \frac{1}{N} \sum_{n=1}^{N} \lfn(x_n).$$  An estimate of the variance of $\hat{Z}$ is given by the standard error of $\lfn({\bf x})$.

%\paragraph*{Annealed Importance Sampling} \citep{neal2001annealed} is a popular thermodynamic integration method which 
There exist several other standard methods for computing the evidence, such as annealed importance sampling (\acro{ais}) \citep{neal2001annealed}, nested sampling \citep{skilling2004nested} and bridge sampling.  For a review, see \citet{chen2000monte}. These typically involve the construction of a markov chain for generating samples, leading to them being termed \emph{Markov chain Monte Carlo} (\acro{mcmc}) methods. 
...
An alternative approach is to posit a distribution over likelihood functions, and to use observations of the likelihood to infer the distribution for $Z$.
Here the issue of convergence is solved naturally: our posterior variance over $Z$ indicates the uncertainty remaining in the quantity we care about. While standard errors are possible for \acro{mcmc}, typically they rely on the sampler mixing well \citep{flegal2008markov}. As such, these errors can not identify exactly the failure to mix that is of most concern. The distribution of most use for our inference is a Gaussian process, which we now briefly introduce. 
...
Quadrature involves evaluating $f(\lfv)$ at a vector of sample points $\vlfv_s$, giving $\vf\pha_s\deq f(\vlfv_s)$. Often this evaluation is computationally expensive; the consequent sparsity of our samples introduces uncertainty about the function $f$ between them, and hence uncertainty about the integral $Y$.
...
for $Z$, forcing $Z$ to be affine in $\tr$. 
...
For brevity, we assume that if we condition on $Z_0$, we will also implicitly condition on its functional derivatives. This allows us to write our mean for  $If$
%
\begin{align}
& \mean{\If}{Z_0,\tvr_s} \nonumber\\
...
& = \mean{\If}{\vr_s} + \iint \mean{\lfn(\lfv)}{\vr_s}\,
...
\paragraph{Methods:} We firstly compared against simple Monte Carlo (\acro{smc}), as described in Section \ref{sec:ev}. As an alternative Monte Carlo technique, we also implemented Annealed Importance Sampling (\acro{ais}).  The inverse temperature schedule was linear as in \citep{BZMonteCarlo}, and the proposal width was adjusted to attain approximately a 50\% acceptance rate.  In an attempt to be as generous to \acro{ais} as possible, the posterior variance of the \acro{ais} estimate of $Z$ was set to the empirical variance over 10 chains.
...
We then tested three novel methods. Firstly, Bayesian Quadrature (\acro{bq}*), which employed the linearisation approach of Section \ref{sec:model_lik} to modeling the log-transformed likelihood values, and the approach of Section \ref{sec:marginalizing} to approximately marginalising hyperparameters (the latter denoted by the asterisk). The samples supplied to it were drawn from the same \acro{ais} chain as used above. The performance of this method allows us to quantify to what extent our innovations improve estimation given a fixed set of samples. Note that marginalising hyperparameters affects only our predictive variance, so its impact can be measured in the relative performance of this approach in $\acro{nmse}$ and $\mathcal{C}$ metrics. 

Secondly, we tested Doubly Bayesian Quadrature (\acro{bbq}), which again used a \gpb over the log-transformed likelihood values. This algorithm also chose samples actively, as described in Section \ref{sec:BBQ}; \eqref{eq:exp_var} was minimised using a multi-start local optimizer, where the starting points were chosen as $\lfv_c$. Finally, \acro{bbq}* was the same algorithm, but with hyperparameters  approximated marginalised. Both algorithms demonstrate the influence of active sampling on our performance; the relative performances of \acro{bbq} and \acro{bbq}* illustrate the importance of hyperparameter marginalisation. 
