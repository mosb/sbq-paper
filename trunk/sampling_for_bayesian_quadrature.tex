...
$
\mean{\tr\st}{\tr_s} = \mean{\tr\st}{\tr_s, \hat{w}}
$.   
The marginal posterior variance is 
\begin{align}
C(\tr\st,\tr\st'|\tr_s) & = C(\tr\st,\tr\st'|\tr_s, \hat{w}) \nonumber\\
& \hspace{-0.5cm} + 
...
This implies that our posterior mean for $Z$ is simply $\mean{\If}{Z_0,\tvr_s,\Delta_c, w_m}$ of the form \eqref{eq:post_mean}. The variance
\begin{align}
  &\cov{\If}{Z_0,\tvr_s,\Delta_c}=
 \iint m_{\lfn|s}(\lfv)\, m_{\lfn|s}(\lfv')\\
&  
\hspace{0.5cm}\biggl(C_{\tr|s}(\lfv,\lfv') + 
\pderiv{m_{\tr|s,\hat{w}}}{w}
\,C_w\,
\pderiv{m_{\tr|s,\hat{w}}}{w}
\biggr)
