...
We propose a novel Bayesian Quadrature approach to the numerical integration problem. Bayesian Quadrature is a model-based means of numerical integration, which, relative to standard Monte Carlo methods, offers sample efficiency and a more robust estimate of the uncertainty in the integral. We extend existing Bayesian Quadrature approaches by introducing a positivity constraint for the integrand and by approximately marginalising over the model's hyperparameters in closed form. We also introduce an active learning scheme to select our function samples, rather than running a Markov chain. We demonstrate our method on a real model comparison problem from the biological sciences.
\end{abstract} 
...
%	\centering
	...
	\caption{The posterior mean plus or minus two \acro{sd}s over $Z$ for different methods over a one-dimensional example, compared to the true $Z$ (in black).}
\label{fig:meanZs}
\end{figure}
...
\begin{figure}
	\centering
	\psfragfig[width=8cm,height=6cm]{figures/plots/log_of_truth_plot_two_hills_1d_nice1}
	\caption{The posterior mean plus or minus two \acro{sd}s over $Z$ for different methods over a one-dimensional example (colours identical to those in Figure \ref{fig:meanZs}), compared to the true $Z$ (in black).}
\label{fig:nll}
\end{figure}
...
The performance of methods tested is illustrated in Tables \ref{tbl:Combined Synthetic Results} and \ref{tbl:Combined Prawn Results}, and in Figures \ref{fig:meanZs} and \ref{fig:nll}. Note that the mean predictions
I have a little paragraph in mind about the results, basically saying that we're overconfident, probably due to some feature of real problems that's poorly modeled by a stationary GP or something like that
